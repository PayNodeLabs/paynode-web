...
const Navbar = () => (
  <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-black/50 backdrop-blur-md px-6 py-4 flex items-center">
    ...
    <div className="hidden md:flex gap-8 text-sm text-gray-400 font-medium">
      <a href="#methods" className="hover:text-[#00ff88] transition-colors whitespace-nowrap">Architecture</a>
        ...
        <div className="inline-flex bg-white/5 rounded-xl p-1 border border-white/10">
          {tabs.map((tab) => (
            <button
              ...
              className={`relative px-6 py-3 rounded-lg text-sm font-medium transition-all ${paymentMethod === tab.id
          ...
          <div className="flex flex-col gap-4">
            ...
            {paymentMethod === "async" ? (
              ...
              <button
                ...
                className={`px-6 py-4 font-bold rounded-xl transition-all text-center ${isSimulating
                  ? "opacity-50 cursor-not-allowed"
                  : paymentMethod === "eip3009"
                    ? "bg-[#00ff88]/10 border border-[#00ff88]/30 text-[#00ff88] hover:bg-[#00ff88]/20 animate-pulse"
                    : "bg-blue-500/10 border border-blue-500/30 text-blue-400 hover:bg-blue-500/20 animate-pulse"
                  }`}
              >
                {isSimulating ? "Simulating..." : `Execute ${paymentMethod === "eip3009" ? "EIP-3009" : "On-chain"} Payment`}
              </button>
            )}
          </div>
        </div>
...
const BackToTop = () => {
  ...
  return (
    <button
      ...
      className="fixed bottom-8 right-8 z-50 p-4 rounded-full bg-black/50 backdrop-blur-md border border-[#00ff88]/30 text-[#00ff88] hover:bg-black/80 hover:scale-110 transition-all shadow-[0_0_20px_rgba(0,255,136,0.1)] group"
    >
